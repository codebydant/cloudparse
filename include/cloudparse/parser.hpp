 ...
 *
 * it implements a factory design pattern to create a parser from a concrete
 * class. each concrete parser is derived from an abstract class InterfaceParser
 * and this module provides a ParserFactory class to register new formats.
 *
 * ParserCloudFile is the common interface to load a cloud file given a path.
 * This interface will check if the current file extension is supported by the
 * factories list. In case there is a provided format, a parser object extension
